//! Verifies that an automatic compaction publishes its checkpoint durably,
//! so a run that fails after compacting can be resumed from the compacted
//! state instead of re-compacting or losing the summary.
...
use cursor_server::{
    ...
    model::{
        ModelConfigInput, ModelType, NewLlmCall, ProjectedMessage, Role, Usage,
...
async fn compacted_checkpoint_survives_followup_error_and_resume_continues() {
    ...
    // A 400-class provider rejection is non-retryable, so the run fails right
    // after the compaction instead of burning retries.
    provider.push_error(Error::Provider(
        "400 Bad Request: post-compaction failure".into(),
    ));
    ...
    // The checkpoint's context token count tracks the provider's
    // context_input_tokens, not the input+output total.
    assert_eq!(first_used, 132_000);
    ...
    store
        .record_llm_usage(
            ...
            Usage {
                ...
                context_input_tokens: Some(132_000),
