...
export function CursorModelEditor({ draft, modelOptions, discovering, onChange, onDiscover }: {
  ...
  return <div className={styles.editor}>
    ...
    <div className={styles.grid}>
      ...
      <FormField label={"Context window tokens"} hint={"Leave blank to use the default."}> <TextInput type="number" min={1} step={1} placeholder={"Leave blank to use the default"} value={draft.model.context_window_tokens ?? ""} onChange={(event) => setModel({ context_window_tokens: numberValue(event.target.value) })} /></FormField>
      {draft.model.type === "openai" ? <>
        <FormField label={"Maximum output tokens"} hint={"Leave blank to use the default."}> <TextInput type="number" min={1} step={1} placeholder={"Leave blank to use the default"} value={draft.model.max_completion_tokens ?? ""} onChange={(event) => setModel({ max_completion_tokens: numberValue(event.target.value) })} /></FormField>
        ...
        <FormField label={"Maximum output tokens"} hint={"Leave blank to use the default."}> <TextInput type="number" min={1} step={1} placeholder={"Leave blank to use the default"} value={draft.model.anthropic_max_tokens ?? ""} onChange={(event) => setModel({ anthropic_max_tokens: numberValue(event.target.value) })} /></FormField>
