  ...
  return <div className={styles.editor}>
    <div className={styles.grid}>
      ...
      <FormField label={"Effort options"} hint={"Comma-separated effort values supported by this model."}>
        <TextInput aria-label={"Effort options"} value={effort.text} disabled={busy} onChange={(event) => setEffort({ text: event.target.value, options: parseOptions(event.target.value) })} />
      </FormField>
      <FormField label={"Context options"} hint={"Comma-separated context values supported by this model, such as 200k, 1m."}>
        <TextInput aria-label={"Context options"} value={context.text} disabled={busy} onChange={(event) => setContext({ text: event.target.value, options: parseOptions(event.target.value) })} />
      </FormField>
