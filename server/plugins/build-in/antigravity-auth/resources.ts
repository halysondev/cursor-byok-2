...
import { CLIENT_ID, CLIENT_SECRET, GOOGLE_TOKEN_URL } from "./google_oauth.ts";
...
const REFRESH_TOKEN_URL = GOOGLE_TOKEN_URL;
...
export async function refreshAccount(
  ...
  return await prepareAccount(resource, null, context) ?? { state: { status: "ready" } };
}
...
export async function prepareAccount(
  ...
  if (refreshToken && isJwtExpired(accessToken)) {
    ...
    expiresAtMs = Date.now() + expiresIn * 1000;
      ...
      // On network glitches keep the existing token; quota refresh retries later.
    }
  }
