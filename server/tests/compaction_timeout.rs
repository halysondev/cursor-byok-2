//! Verifies that a stalled automatic compaction is bounded by the engine's
//! own deadline: the model call is cancelled, the truncated-history fallback
//! summary is published, and the run continues.
...
use cursor_server::{
    ...
    model::{ModelConfigInput, ModelType, Role, Usage, OPENAI_CHAT_ENDPOINT},
...
async fn stalled_auto_compaction_times_out_falls_back_and_continues() {
    ...
    // Minimum reserve: the 100K window then budgets 50K, so the small seed
    // turn fits while the 300K answer overflows it on the resume request.
    store
    ...
    provider.push(text_response("x".repeat(300_000), 1_000, 25_000));
    // The automatic summary request never produces an event. The engine must
    // stop waiting at its own deadline instead of relying on provider
    // cancellation.
    ...
    assert_eq!(
        requests[1].model.max_output_tokens,
        Some(cursor_server::run::compaction_output_tokens())
    );
...
fn history_text(history: &[cursor_server::model::ProjectedMessage]) -> String {
    use cursor_server::model::{ContentPart, ProjectedContent};
