...
export function resolveModelRoute(model: ModelSnapshot, effort: string | null): Route {
  ...
  if (routes.length !== stored.length || keys.size !== 1 || !keys.has(versionKey(model.id))) {
    ...
    // No stored routes (snapshot synced by an older build): call the model id
    // directly at the highest documented tier defaults.
    const tier = splitTier(model.id).tier;
    return {
      id: model.id,
      tier,
      thinkingBudget: 0,
