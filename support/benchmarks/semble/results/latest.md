# Semble vs CodeGraph React/Vue comparison benchmark

- Environment: macos aarch64, rustc 1.97.1 (8bab26f4f 2026-07-14)
- Queries: Top 10, each repeated 5 times
...
| Dataset | System | Cold-start ready | Cached load | Natural language P50 / P95 / σ | Literal P50 / P95 / σ | Cached symbol P50 / P95 / σ | Refresh+symbol P50 / P95 | Files / index units | Index size |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
| Dataset | System | Track | Recall@1 / @5 / @10 | MRR@10 | nDCG@10 |
| --- | --- | --- | ---: | ---: | ---: |
...
## react · Semble · natural_language details

| Query | First hit | P50 / P95 / σ | Top 1 |
| --- | ---: | ---: | --- |
...
## react · CodeGraph · natural_language details

| Query | First hit | P50 / P95 / σ | Top 1 |
| --- | ---: | ---: | --- |
| `state-hook-api` | miss | 204.85 / 211.44 / 3.00 ms | `compiler/packages/babel-plugin-react-compiler/src/ReactiveScopes/PruneNonEscapingScopes.ts:202-417` |
| `create-context` | miss | 248.40 / 251.60 / 2.98 ms | `compiler/crates/react_compiler_hir/src/environment_config.rs:1-230` |
| `schedule-fiber-update` | miss | 177.86 / 180.33 / 1.97 ms | `compiler/packages/snap/src/reporter.ts:117-181` |
| `commit-root` | miss | 257.56 / 306.25 / 19.81 ms | `compiler/packages/babel-plugin-react-compiler/src/Utils/Result.ts:115-201` |
| `suspense-begin-work` | miss | 230.27 / 232.95 / 1.73 ms | `compiler/packages/babel-plugin-react-compiler/src/Flood/Types.ts:17-946` |
| `delegated-dom-events` | miss | 274.57 / 275.35 / 1.58 ms | `packages/internal-test-utils/ReactInternalTestUtils.js:349-379` |
| `create-dom-root` | miss | 211.90 / 214.24 / 1.10 ms | `compiler/packages/babel-plugin-react-compiler/src/Inference/InferMutationAliasingRanges.ts:599-619` |
| `reconcile-keyed-array` | miss | 226.78 / 242.64 / 7.47 ms | `compiler/packages/babel-plugin-react-compiler/src/Flood/Types.ts:17-946` |
| `dispatch-state-update` | miss | 183.96 / 203.61 / 9.03 ms | `compiler/packages/snap/src/reporter.ts:1-275` |
| `hydrate-dom-root` | miss | 268.40 / 272.53 / 2.65 ms | `packages/react-dom/src/client/ReactDOMRoot.js:149-357` |

## react · CodeGraph · literal details

| Query | First hit | P50 / P95 / σ | Top 1 |
| --- | ---: | ---: | --- |
| `state-hook-api` | miss | 202.23 / 224.65 / 9.12 ms | `compiler/packages/babel-plugin-react-compiler/src/Flood/Types.ts:148-865` |
| `create-context` | miss | 226.69 / 231.59 / 3.10 ms | `compiler/crates/react_compiler_hir/src/object_shape.rs:305-433` |
| `schedule-fiber-update` | 1 | 233.96 / 235.11 / 1.69 ms | `packages/react-reconciler/src/ReactFiberWorkLoop.js:699-1132` |
| `commit-root` | miss | 200.31 / 202.11 / 1.09 ms | `packages/react-dom/src/client/ReactDOMRoot.js:149-357` |
| `suspense-begin-work` | miss | 246.94 / 250.10 / 2.24 ms | `packages/react-reconciler/src/ReactFiberRootScheduler.js:572-582` |
| `delegated-dom-events` | miss | 287.21 / 294.48 / 3.28 ms | `packages/react-dom/src/client/ReactDOMRoot.js:149-357` |
| `create-dom-root` | miss | 248.93 / 250.32 / 1.06 ms | `packages/react-dom/src/client/ReactDOMRoot.js:149-357` |
| `reconcile-keyed-array` | miss | 291.47 / 298.40 / 3.24 ms | `compiler/crates/react_compiler_hir/src/environment.rs:38-534` |
| `dispatch-state-update` | miss | 238.16 / 240.85 / 4.42 ms | `compiler/crates/react_compiler_inference/src/infer_mutation_aliasing_effects.rs:117-144` |
| `hydrate-dom-root` | miss | 211.90 / 216.60 / 2.41 ms | `packages/react-dom-bindings/src/shared/ReactFlightClientConfigDOM.js:1-139` |

## react · CodeGraph · symbol details

| Query | First hit | P50 / P95 / σ | Top 1 |
| --- | ---: | ---: | --- |
| `state-hook-api` | miss | 2.51 / 3.14 / 0.29 ms | `compiler/packages/babel-plugin-react-compiler/src/__tests__/fixtures/compiler/globals-dont-resolve-local-useState.js:4-7` |
| `create-context` | miss | 2.78 / 3.19 / 0.19 ms | `compiler/apps/playground/lib/createContext.ts:26-40` |
| `schedule-fiber-update` | 1 | 2.14 / 2.57 / 0.23 ms | `packages/react-reconciler/src/ReactFiberWorkLoop.js:987-1113` |
| `commit-root` | miss | 2.23 / 2.84 / 0.29 ms | `packages/react-reconciler/src/ReactFiberCommitWork.js:220-238` |
| `suspense-begin-work` | miss | 12.34 / 12.72 / 0.27 ms | `—` |
| `delegated-dom-events` | 1 | 2.34 / 2.78 / 0.27 ms | `packages/react-dom-bindings/src/events/DOMPluginEventSystem.js:432-459` |
| `create-dom-root` | miss | 2.68 / 2.94 / 0.26 ms | `packages/use-sync-external-store/src/__tests__/useSyncExternalStoreShared-test.js:90-110` |
| `reconcile-keyed-array` | miss | 14.48 / 15.25 / 0.65 ms | `—` |
| `dispatch-state-update` | miss | 12.23 / 13.05 / 0.40 ms | `—` |
| `hydrate-dom-root` | miss | 2.14 / 2.40 / 0.11 ms | `packages/react-dom/src/__tests__/ReactDOMFizzShellHydration-test.js:104-123` |

## vue · Semble · natural_language details
...
| `compile-template` | miss | 1.91 / 1.95 / 0.04 ms | `packages/compiler-sfc/src/compileTemplate.ts:105-107` |
...
## vue · CodeGraph · natural_language details
...
| `patch-keyed-children` | miss | 50.77 / 51.52 / 0.92 ms | `packages/runtime-core/src/renderer.ts:153-2640` |
| `compile-template` | miss | 56.67 / 58.12 / 0.74 ms | `packages/runtime-core/src/compat/global.ts:146-156` |
| `parse-template` | miss | 49.11 / 49.73 / 0.43 ms | `packages/compiler-sfc/src/parse.ts:22-354` |
| `reactivity-watch` | miss | 47.20 / 47.46 / 0.48 ms | `packages/reactivity/src/watch.ts:34-93` |
| `create-app-api` | miss | 62.82 / 63.58 / 0.45 ms | `packages/runtime-core/src/compat/global.ts:85-156` |
| `define-component` | 2 | 61.96 / 62.26 / 0.18 ms | `packages/runtime-core/src/compat/global.ts:64-333` |
| `hydrate-node` | miss | 61.93 / 63.06 / 0.67 ms | `packages/runtime-core/src/compat/global.ts:146-156` |

## vue · CodeGraph · literal details
...
| `create-renderer` | miss | 50.20 / 50.30 / 0.26 ms | `packages/runtime-core/src/renderer.ts:93-304` |
| `patch-keyed-children` | miss | 41.23 / 41.83 / 0.32 ms | `packages-private/dts-test/ref.test-d.ts:345-351` |
| `compile-template` | 1 | 39.40 / 40.28 / 0.38 ms | `packages/compiler-core/src/compile.ts:1-122` |
| `parse-template` | miss | 40.95 / 41.30 / 0.25 ms | `packages/compiler-core/src/parser.ts:1-288` |
| `reactivity-watch` | miss | 37.55 / 38.09 / 0.54 ms | `packages/runtime-core/src/apiWatch.ts:1-92` |
| `create-app-api` | 1 | 40.47 / 40.94 / 0.46 ms | `packages/runtime-core/src/apiCreateApp.ts:30-494` |
| `define-component` | miss | 43.52 / 45.06 / 0.66 ms | `packages/shared/src/general.ts:1-219` |
| `hydrate-node` | miss | 45.72 / 47.40 / 0.78 ms | `packages/runtime-core/src/vnode.ts:71-256` |

## vue · CodeGraph · symbol details
...
| `patch-keyed-children` | miss | 1.96 / 4.02 / 0.82 ms | `—` |
...
| `hydrate-node` | miss | 0.27 / 0.37 / 0.05 ms | `packages/runtime-core/src/components/Suspense.ts:806-856` |
